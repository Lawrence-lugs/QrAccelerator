...
def generate_top_inputs_from_onnx(
    savepath,
    nx_model,
    node_number,
    nx_input_dict,
    ...
    nodes = cnodes.get_cnode_from_onnx_node(
        node = nx_model.graph.node[node_number],
        nx_model = nx_model,
    )

    input_tensor_name = nodes[0].inputs[0]
    output_tensor_name = nodes[1].outputs[0]

    tensor_in = onnx_utils.get_intermediate_tensor_value(
        nx_model, 
        tensor_name= input_tensor_name,
        input_dict = nx_input_dict
    )

    # input_name = onnx.load(nx_model).graph.input[0].name
    ...
    t_toeplitz = compute.toeplitzize_input(tensor_in.squeeze(axis=0))    

    t_matrix = nodes[0].matrix
    t_ifmap = tensor_in
    ...
    q_out = (ref * int_scale[None,:,None,None])
